...
// use serde_json::Value;  // 关键：用于 to_value
// use std::sync::Arc;
...
//     Json(payload): Json<LACreate>,
// ) -> Result<RespJson<Value>, (StatusCode, String)> {
...
//         .map_err(|_| (StatusCode::BAD_REQUEST, "Invalid audience_id".into()))?;
...
//             "is_present": false,
//             "joined_at": BsonDateTime::from_millis(Utc::now().timestamp_millis()),
//         },
//         None,
//     )
//     .await
//     .map_err(|_| (StatusCode::INTERNAL_SERVER_ERROR, "加入失败".into()))?;

//     Ok(RespJson(serde_json::json!({ "message": "加入成功" })))
// }

// // DELETE /la/delete?lecture_id=..&audience_id=..
// async fn delete_la(
//     State(client): State<AppState>,
//     Query(params): Query<std::collections::HashMap<String, String>>,
// ) -> Result<RespJson<Value>, (StatusCode, String)> {
//     let coll = la_collection(&client);
//     let lecture_id = params.get("lecture_id").ok_or((StatusCode::BAD_REQUEST, "缺少 lecture_id".into()))?;
//     let audience_id = params.get("audience_id").ok_or((StatusCode::BAD_REQUEST, "缺少 audience_id".into()))?;
...
//     }
//     Ok(RespJson(serde_json::json!({ "message": "删除成功" })))
// }

// // GET /la/by-lecture?lecture_id=..
// async fn get_by_lecture(
//     State(client): State<AppState>,
//     Query(params): Query<std::collections::HashMap<String, String>>,
// ) -> Result<RespJson<Vec<Value>>, (StatusCode, String)> {
//     let coll = la_collection(&client);
//     let lecture_id = params.get("lecture_id").ok_or((StatusCode::BAD_REQUEST, "缺少 lecture_id".into()))?;
//     let lecture_oid = ObjectId::parse_str(lecture_id)
//         .map_err(|_| (StatusCode::BAD_REQUEST, "Invalid lecture_id".into()))?;
...
//     })? {
//         let mut v = serde_json::to_value(&doc).map_err(|_| {
//             (StatusCode::INTERNAL_SERVER_ERROR, "序列化错误".into())
//         })?;
//         if let Value::Object(ref mut map) = v {
//             map.insert("id".into(), Value::String(map["_id"].as_str().unwrap().into()));
//             map.remove("_id");
//             map.insert(
//                 "lecture_id".into(),
//                 Value::String(map["lecture_id"].as_str().unwrap().into()),
//             );
//             map.insert(
//                 "audience_id".into(),
//                 Value::String(map["audience_id"].as_str().unwrap().into()),
//             );
//         }
//         list.push(v);
//     }
...
// // GET /la/by-audience?audience_id=..
...
//     let mut cursor = coll
//         .find(doc! { "audience_id": audience_oid }, None)
//         .await
//         .map_err(|_| (StatusCode::INTERNAL_SERVER_ERROR, "查询失败".into()))?;
...
//         )
//         .await
//         .map_err(|_| (StatusCode::INTERNAL_SERVER_ERROR, "查询 LA 失败".into()))?;

//     let mut user_ids = Vec::new();
//     while let Some(doc) = cursor.try_next().await.map_err(|_| {
//         (StatusCode::INTERNAL_SERVER_ERROR, "读取 LA 失败".into())
//     })? {
...
//     let mut users_cursor = user_coll
//         .find(
//             doc! { "_id": { "$in": user_ids } },
//             Some(mongodb::options::FindOptions::builder()
//                 .projection(doc! {
//                     "username": 1,
//                     "email": 1,
//                     "gender": 1,
...
//     while let Some(doc) = users_cursor.try_next().await.map_err(|_| {
//         (StatusCode::INTERNAL_SERVER_ERROR, "读取用户失败".into())
//     })? {
//         let mut v = serde_json::to_value(&doc).map_err(|_| {
//             (StatusCode::INTERNAL_SERVER_ERROR, "序列化错误".into())
//         })?;
//         if let Value::Object(ref mut map) = v {
//             map.insert("id".into(), map["_id"].clone());
//             map.remove("_id");
//         }
//         users.push(v);
//     }

//     Ok(RespJson(serde_json::json!({ "users": users })))
// }

// // PATCH /la/update_is_present?lecture_id=..&audience_id=..
// async fn update_is_present(
//     State(client): State<AppState>,
//     Query(params): Query<std::collections::HashMap<String, String>>,
//     Json(payload): Json<LAUpdatePresent>,
// ) -> Result<RespJson<Value>, (StatusCode, String)> {
//     let coll = la_collection(&client);
//     let lecture_id = params.get("lecture_id").ok_or((StatusCode::BAD_REQUEST, "缺少 lecture_id".into()))?;
//     let audience_id = params.get("audience_id").ok_or((StatusCode::BAD_REQUEST, "缺少 audience_id".into()))?;
...
// // GET /la/lectures_by_user/{user_id}
// async fn get_lectures_by_user(
//     State(client): State<AppState>,
//     Path(user_id): Path<String>,
// ) -> Result<RespJson<Vec<Value>>, (StatusCode, String)> {
//     let coll = la_collection(&client);
//     let audience_oid = ObjectId::parse_str(&user_id)
...
//         .route("/update_is_present", patch(update_is_present))
